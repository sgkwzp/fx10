...
深度卷积神经网络 (DCNN) 在合成孔径雷达 (SAR) 目标检测中取得了卓越的性能，但这是以巨大的计算资源为代价的，部分原因是在单个卷积层中提取了冗余特征。
近期的研究要么深入研究模型压缩方法，要么专注于精心设计的轻量级模型，而这两种方法都会导致性能下降。本文提出了一种用于 SAR 目标检测的高效卷积模块 SFS-Conv，该模块通过分流-感知-选择策略增加了每个卷积层内的特征多样性。
具体而言，我们将输入特征图分流到空间和频率两个方面。前者通过动态调整感受野来感知各种目标的上下文，而后者通过分数阶 Gabor 变换来捕捉丰富的频率变化和纹理特征。
为了自适应地融合空间和频率方面的特征，我们提出了一个无参数特征选择模块，以确保保留最具代表性和独特性的信息。基于 SFS-Conv，我们构建了一个轻量级 SAR 目标检测网络 SFS-CNet。
实验结果表明，SFS-CNet 在一系列 SAR 物体检测基准上优于最先进的 (SoTA) 模型，同时降低了模型尺寸和计算成本。
"""
...
if __name__ == '__main__':
    channel = 32
    height = width = 20
    block = SFSConv(channel, channel).to('cuda')
    input = torch.rand(1, 32, 4, 4).to('cuda')
    output = block(input)
    print(input.size())
    print(output.size())