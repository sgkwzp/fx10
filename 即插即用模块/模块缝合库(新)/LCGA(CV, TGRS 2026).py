...
class WindowAttention(nn.Module):
    def __init__(self, dim, idx, split_size=[8, 8], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0.,
        ...
        self.curvature_conv = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim, bias=False)  # 用来提取局部曲率代理
        self.alpha = nn.Parameter(torch.zeros(1))  # 控制曲率对 logits 的调制强度
        self.beta = nn.Parameter(torch.tensor(0.5))  # 控制标准注意力和曲率注意力的融合比例
