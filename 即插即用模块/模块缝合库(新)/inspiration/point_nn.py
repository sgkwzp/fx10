...
class FPS_kNN(nn.Module):
    ...
    def forward(self, xyz, x):
        ...
        # 生成 batch 数组，每个点的批次索引
        batch_indices = torch.arange(B, device=xyz.device).repeat_interleave(N)

        # FPS
        fps_idx = fps(xyz, ratio=self.group_num / N, batch=batch_indices)
        fps_idx = fps_idx.view(B, self.group_num)

        # 从 xyz 和 x 中按 fps_idx 取样
