...
class EuclidDistance_Assign_Module(nn.Module):
    def __init__(self, feature_dim=512, cluster_num=64, maxpool=1, soft_assign_alpha=32.0):
    ...
    def forward(self, x, alpha=None):
        ...
        x_distance = soft_assign.view(x_transpose.shape[0], x_transpose.shape[1], x_transpose.shape[2],
                                      self.cluster_num)
        x_distance_assign = self.assign_func(x_distance, alpha)
        return x_distance, x_distance_assign

if __name__ == '__main__':
    # Define the input tensor with a suitable shape
    input_tensor = torch.rand((2, 512, 16, 16))  # Example shape (batch_size, feature_dim, height, width)

    # Initialize the module
    block = EuclidDistance_Assign_Module()

    # Forward pass
    x_distance, x_distance_assign = block(input_tensor)

    # Print output shapes
    print("Input size:", input_tensor.size())
    print("x_distance size:", x_distance.size())
    print("x_distance_assign size:", x_distance_assign.size())
