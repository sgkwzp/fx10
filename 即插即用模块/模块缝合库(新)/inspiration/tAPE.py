...
class tAPE(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=16, scale_factor=1.0):
...
if __name__ == '__main__':
    block = tAPE(d_model=64)
    # input = torch.rand(20, 32, 512) # [sequence length, batch size, embed dim]
    input = torch.rand(36*36, 16, 64) # [sequence length, batch size, embed dim]
    output = block(input)
    print(input.size())
    print(output.size())
