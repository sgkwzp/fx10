# B站UP：箫张跋扈(https://space.bilibili.com/478113245?spm_id_from=333.1007.0.0)整理
...
"""
以卷积神经网络（CNN）和视觉变换器（ViT）为基础模块的变体模型在图像已经得到了非常广泛的开发和应用。
然而，CNN处理长信息序列的能力往往受到限制，而ViT对局部特征信息的低敏感性和二次计算复杂度的问题限制了其发展。
近年来，状态空间模型（SSM），特别是二维选择性扫描（SS2D）的出现，对传统 CNN 和 ViT 作为视觉神经网络基础模块的长期主导地位产生了影响。
提出的高阶2D选择性扫描（H-SS2D）通过高阶交互逐步减少SS2D操作期间冗余信息的引入。
"""
...
if __name__ == '__main__':
    # 输入图像尺寸为[批次大小, 通道数, 高度, 宽度]
    batch_size = 4  # 批次大小
    in_channels = 128  # 输入通道数，例如RGB图像
    height = 32  # 图像的高度
    width = 32  # 图像的宽度

    # 初始化H_SS2D模块
    # 需要提供的参数包括dim（模块的维度），order（Hierarchical Selective-Scan SSM的顺序）
    # 示例，根据实际情况调整参数
    dim = 128  # 模型的维度
    order = 5
    h_ss2d_block = H_SS2D(dim=dim, order=order, h=height // 2, w=width // 2, s=1.0, d_state=16)

    # 将模块转移到合适的设备上
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    h_ss2d_block = h_ss2d_block.to(device)

    # 生成随机输入张量，尺寸为[batch_size, in_channels, height, width]
    input_tensor = torch.rand(batch_size, in_channels, height, width).to(device)

    # 通过模块传递输入获取输出
    output_tensor = h_ss2d_block(input_tensor)
