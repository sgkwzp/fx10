...
"""
在相关的时间序列数据中对复杂的空间和时间相关性进行建模对于理解交通动态和预测不断发展的交通系统的未来状态是必不可少的。最近的工作重点是设计复杂的图神经网络架构，以在预定义图的帮助下捕获共享模式。
在本文中，我们认为学习特定于节点的模式对于流量预测至关重要，而预定义的图是可以避免的。
为此，我们提出了两个自适应模块，用于增强图卷积网络（GCN）的新功能：1）节点自适应参数学习（NAPL）模块，用于捕获特定于节点的模式;2）数据自适应图生成（DAGG）模块，自动推断不同流量序列之间的相互依赖关系。
我们进一步提出了一种自适应图卷积递归网络（AGCRN），以基于两个模块和递归网络自动捕获流量序列中的细粒度空间和时间相关性。
我们对两个真实世界交通数据集的实验表明，在没有关于空间连接的预定义图表的情况下，AGCRN的性能明显优于最先进的技术。
"""
