...
class CenterPixelAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        k_self_avg = rearrange(k_self_avg, 'b n (h d) -> b h d n',
                               h=h)  # k_self_avg.view(b, 1, h, self.dim_heads).transpose(1, 2)  # [b, h, 1, dim_heads]

        # 交换Q的最后两个维度，然后与K相乘得到B h d d的光谱注意力
        # q_center_swapped = q_center.transpose(-1, -2)  # [b, h, dim_heads, 1]
        spectral_attn = q_center @ k_self_avg.transpose(-1,
                                                        -2)  # torch.einsum('bhdi,bhdj->bhij', q_center_swapped, k_self_avg)  # [b, h, dim_heads, dim_heads]
...
if __name__ == '__main__':
    ...
    input = torch.rand(2, 16, 64).to(device)
    output = block(input)
