...
def print_model_params(model):
    for name, param in model.named_parameters():
        logging.info(name, param.min().item(), param.max().item(), param.mean().item())
    return
...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv2d, torch.nn.Conv1d, MaskedConv1D, torch.nn.Embedding, torch.nn.LSTM)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            elif re.search(r'weight_ih_l\d$', pn) or re.search(r'weight_hh_l\d$', pn):
                decay.add(fpn)
            # 检查偏置参数，匹配以 'bias_ih_l' 开头，后接任意数字，并以 '0' 结尾的模式
            elif re.search(r'bias_ih_l\d$', pn) or re.search(r'bias_hh_l\d$', pn):
                no_decay.add(fpn)
            elif pn.endswith('layer_norm.weight'):
                no_decay.add(fpn)
            elif 'expected_feat_predictor' in fpn:
                if pn.endswith('weight'):
                    decay.add(fpn)
                elif pn.endswith('bias'):
                    no_decay.add(fpn)
                elif pn.endswith('prompt'):
                    no_decay.add(fpn)
...
def train_one_epoch(
    ...
    tb_writer = None,
    print_freq = 10,
    cluster_accumulate_method = None,
):
    ...
    if curr_epoch == -1 and model.module.use_rebuild_expected_feat:
        if cluster_accumulate_method is None:
            cluster_accumulate_method_str = ''
        elif cluster_accumulate_method == 'pred_segment_gt_label':
            cluster_accumulate_method_str = 'cluster_accumulate_method_pred_segment_gt_label'
        elif cluster_accumulate_method == 'gt':
            cluster_accumulate_method_str = 'cluster_accumulate_method_gt'

        model.eval()
        with torch.no_grad():
            model.module.action_clusters.clear_feats()
            for iter_idx, video_list in enumerate(train_loader, 1):
                if iter_idx < len(train_loader):
                    model(video_list, mode=f'accumulate_transformed_feat {cluster_accumulate_method_str}')
                else:
                    model(video_list, mode=f'accumulate_transformed_feat {cluster_accumulate_method_str} cluster_transformed_feat')

        logging.info('Build Cluster Done!')
        return
    ...
    for iter_idx, video_list in enumerate(train_loader):
        ...
        if (iter_idx != 0) and (iter_idx % print_freq) == 0:
            ...
            logging.info('\t'.join([block1, block2, block3, block4]))

    # finish up and print
    lr = scheduler.get_last_lr()[0]
    logging.info("[Train]: Epoch {:d} finished with lr={:.8f}\n".format(curr_epoch, lr))
