...
class ANETdetection(object):
    ...
    def _get_predictions_with_label(self, prediction_by_label, label_name, cidx):
        ...
        try:
            ...
            logging.info('Warning: No predictions of label \'%s\' were provdied.' % label_name)
            return pd.DataFrame()
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if verbose:
            # print the results
            logging.info('[RESULTS] Action detection results on {:s}.'.format(
                self.dataset_name)
            )
            block = ''
            ...
            logging.info(block)
            logging.info('Average mAP: {:>4.2f} (%)'.format(average_mAP*100))
