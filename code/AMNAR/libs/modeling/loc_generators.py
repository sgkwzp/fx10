...
class PointGenerator(nn.Module):
    ...
    def forward(self, feats):
        ...
        for feat_len, buffer_pts in zip(feat_lens, self.buffer_points):
            assert feat_len <= buffer_pts.shape[0], f"Reached max buffer length for point generator {self.max_seq_len}, feat_len: {feat_len}, buffer_pts.shape: {buffer_pts.shape}"
