...
class ConvGCNTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(n_in, (list, tuple)):
            ...
            vis_n_embd = (n_embd - graph_n_embd) if self.use_gcn else n_embd
        ...
        for idx in range(arch[0]):
            ...
            if self.online_mode:
                # For online mode, use causal padding (only past information)
                padding = (n_embd_ks-1, 0)  # Causal padding for online mode
            else:
                # Symmetric padding as before
                padding = n_embd_ks // 2
            
            self.embd.append(
                MaskedConv1D(
                    n_in, vis_n_embd, n_embd_ks,
                    stride=1, padding=padding, bias=(not with_ln)
                )
            )
            if with_ln:
